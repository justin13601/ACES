...
class TestQueryFunctions(unittest.TestCase):
    ...
    def test_summarize_window(self):
        ...
        test_temporal_node = Node("gap", endpoint_expr=(False, timedelta(hours=48), True), constraints={"is_admission": (None, 0), "is_death": (None, 0), "is_discharge": (None, 0)})

        with patch('esgpt_task_querying.query.summarize_temporal_window') as mock_summarize_temporal_window:
            with patch('esgpt_task_querying.query.summarize_event_bound_window') as mock_summarize_event_bound_window:
                ...
                test_simple_predicate_cols = [col for col in test_simple_predicates_df.columns if col.startswith('is_')],
        ...
        test_event_bound_node = Node('target', endpoint_expr=(True, None, True), constraints={"is_death_or_is_discharge": (1, None), "is_admission": (1, None)})
